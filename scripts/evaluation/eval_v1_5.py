...
═══════════════════════════════════════════════════════════════════════════════
  v1.5 was rejected: external F1 delta of −0.003 (below the +1.0 pt ship
  criterion). v1.3 retained as final production model. See scripts/train_v1_5.py
  header and outputs/eval/v1.5_candidate*.json for full record.

  Requires re-training v1.5 (via scripts/train_v1_5.py) to populate the
  models/v1.5_candidate/ directory before this evaluator can be run.
═══════════════════════════════════════════════════════════════════════════════

Evaluator for v1.5 (frozen MiniLM sentence embeddings + RF).
...
holding the MiniLM embedding instead of a TF-IDF sparse row.

This works because predict_message hstacks [tfidf, char_tfidf, numerical_scaled]
without caring what "tfidf" internally represents, and the v1.5 model was
trained on exactly [MiniLM(384), char(3000), numerical(26)] concatenation
in the same order.

Rule floors (sections 12b–12i of predict_message) apply unchanged — this
gives v1.5 the same rule-floor-augmented decision path as v1.3, making
...
class MiniLMAsTfidf:
    ...
    def transform(self, texts):
        embs = self.st.encode(list(texts), convert_to_numpy=True)
...
def main():
    ...
    ap.add_argument('--models-dir',  default=f'{BASE}/models/v1.5_candidate')
    ap.add_argument('--external',    action='store_true')
    ap.add_argument('--rung1',       action='store_true')
    ap.add_argument('--sample',      type=int, default=250)
    args = ap.parse_args()

    print(f"Loading v1.5 from {args.models_dir}…")
    pipe = load_v1_5(args.models_dir)
    ...
    if args.rung1:
        ...
        for i, row in enumerate(sample.itertuples()):
            ...
            r = predict_v1_5(row.raw_text, pipe)
        ...
        out = f'{BASE}/outputs/eval/v1.5_candidate.json'
    ...
    if args.external:
        ...
        for i, it in enumerate(items):
            ...
            r = predict_v1_5(it['raw_text'], pipe)
            preds.append(1 if r['verdict'] in ('SCAM','SUSPICIOUS') else 0)
            probs.append(float(r.get('confidence', 0))/100)
        ...
        out = f'{BASE}/outputs/eval/v1.5_candidate_external.json'
        with open(out, 'w') as f:
